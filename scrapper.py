...
def haversine(iata_from: str, iata_to: str) -> Optional[float]:
    origin_coords = AIRPORT_COORDS.get(iata_from)
    destination_coords = AIRPORT_COORDS.get(iata_to)
    ...
    return round(earth_radius_km * 2 * atan2(sqrt(a), sqrt(1 - a)), 1)


def generate_date_pairs(start: datetime, window_days: int, trip_days: int) -> List[Tuple[str, str]]:
    pairs: List[Tuple[str, str]] = []
    for offset in range(window_days):
        outbound = start + timedelta(days=offset)
        inbound = outbound + timedelta(days=trip_days)
        pairs.append((outbound.strftime("%Y-%m-%d"), inbound.strftime("%Y-%m-%d")))
    return pairs
...
def enrich_flights_with_distance(flights: list) -> list:
    for flight in flights:
        ...
        for segment in flight.get("flights", []):
            ...
            dep_id = departure_airport.get("id")
            arr_id = arrival_airport.get("id")

            distance = haversine(dep_id, arr_id) if dep_id and arr_id else None
            segment["distance_km"] = distance
            departure_airport["coords"] = AIRPORT_COORDS.get(dep_id)
            arrival_airport["coords"] = AIRPORT_COORDS.get(arr_id)
